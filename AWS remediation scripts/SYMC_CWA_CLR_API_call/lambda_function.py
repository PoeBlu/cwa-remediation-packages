...
def lambda_handler(event, context):
    try:
        ...
        print(f'Input Message Received : - {json.dumps(input_msg)}')
        ...
        print(f'Error Occured while calling CWA CLosed Loop API {str(ex)}')
        return {
            'statusCode': 500,
            'body': json.dumps(
                f'Error Occured while calling CWA CLosed Loop API {str(ex)}'
            ),
        }
